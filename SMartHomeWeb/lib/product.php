...
function get_list_product_by_catid_brand_id($cat_id, $brand_id) { 
    global $con;
    // $cat_id = mysqli_escape_string($con, $cat_id);
    // $brand_id = mysqli_escape_string($con, $brand_id);
    $sql = "select * from tbl_product where cat_id = {$cat_id} and brand_id = {$brand_id} and status_pro=0";
    $result = mysqli_query($con, $sql); 
    $list_product = array();
    while ($row = mysqli_fetch_array($result)) {
        $list_product[] = $row;
...
function get_brand_by_brandid($brand_id) { 
    global $con;
...
//Lấy danh mục đang bán
function get_category()
{
    global $con;
    $sql = "SELECT cat_id, cat_name FROM tbl_category WHERE status_cat=0";
    $result = mysqli_query($con, $sql);
    $list_cat = array();
    while($row = mysqli_fetch_array($result)){
        $list_cat[] = $row;
...
function get_brand_by_cat_id($cat_id){
    global $con;
    $sql = "SELECT * FROM tbl_brand WHERE cat_id={$cat_id}";
    $result = mysqli_query($con, $sql);
    $list_brand = array();
    while($row = mysqli_fetch_array($result)){
        $list_brand[] = $row;
