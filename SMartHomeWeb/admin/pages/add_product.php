<?php 
    ...
    $product_nm = mysqli_escape_string($con, $product_nm);
    $quantity = mysqli_escape_string($con, $quantity);
    $original_price = mysqli_escape_string($con, $original_price);
    $promotional_price = mysqli_escape_string($con, $promotional_price);
    $short_desc = mysqli_escape_string($con, $short_desc);
    $detail_desc = mysqli_escape_string($con, $detail_desc);
    $cat_id = mysqli_escape_string($con, $cat_id);
    $brand_id = mysqli_escape_string($con, $brand_id);

    //Kiểm tra tính hợp lệ của dữ liệu trước khi submit 
    //* Upload file ảnh
